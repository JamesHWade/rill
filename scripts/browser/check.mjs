...
const output = path.resolve('../../artifacts/responsive-audit');
...
try {
  ...
  assert.equal(await page.locator('.story-actions').first().isVisible(), true, 'Swipe reveals actions');
  assert.equal(await page.locator('#reader-document').count(), 0, 'Swipe does not open article');
  await page.locator('.story-actions-toggle').first().click();
  await gesture('.story-card', [['touchstart', 250, 230], ['touchmove', 240, 300], ['touchend', 100, 305]]);
  assert.equal(await page.locator('.story-actions').first().isVisible(), false, 'Vertical scrolling is ignored');
  await gesture('.story-card', [['touchstart', 10, 230], ['touchend', 200, 230]]);
  assert.equal(await page.locator('.story-actions').first().isVisible(), false, 'Edge gestures are ignored');
  await gesture('.story-card', [['touchstart', 250, 230, 2], ['touchend', 100, 230]]);
  assert.equal(await page.locator('.story-actions').first().isVisible(), false, 'Multiple touches are ignored');
  await page.locator('.story-actions-toggle').first().focus();
  await page.keyboard.press('Enter');
  assert.equal(await page.locator('.story-actions').first().isVisible(), true, 'Keyboard reveals actions');
  await page.locator('.story-actions').first().getByRole('button', {name: 'Save', exact: true}).click();
  await page.locator('.story-actions button[aria-pressed="true"]').first().waitFor();
  assert.equal(await page.evaluate(() => document.activeElement.textContent.trim()), 'Saved', 'Save restores focus after updating the row');
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('.story-actions').first().isVisible(), false, 'Escape closes queue actions');
  assert.equal(await page.evaluate(() => document.activeElement.classList.contains('story-actions-toggle')), true, 'Escape restores the actions button');
  await page.keyboard.press('Enter');
  await page.locator('.story-actions').first().getByRole('button', { name: 'Read', exact: true }).click();
