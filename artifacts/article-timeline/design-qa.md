...
A subsequent automated review found that the explicit card action label hid descendant source context from its accessible name. Cards now reference their excerpt and meaningful image alt text with `aria-describedby`. The browser test inspects Chromium's accessibility tree and records both the action name and complete accessible description in `accessible-card.json`; 270 focused UI/deployment assertions and the timeline browser suite pass.
